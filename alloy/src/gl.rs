...
/// have been created on it. The trailing glFinish guarantees the offscreen
/// render has completed before the texture crosses to the render thread, which
/// samples it from a separate shared GL context (where the contents are
/// otherwise undefined).
pub fn render_display_list_to_texture(
  ...
  unsafe {
    // Save the current bindings so Impeller's cached GL state stays valid.
    ...
    let result = if status != glow::FRAMEBUFFER_COMPLETE {
      ...
      match impeller_ctx.wrap_fbo(fbo.0.get() as u64, PixelFormat::RGBA8888, size) {
    ...
    match result {
      Ok(()) => impeller_ctx
        .adopt_opengl_texture(alloc_width as u32, alloc_height as u32, 1, tex.0.get() as u64)
