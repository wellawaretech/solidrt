...
declare module "flux:path" {
   ...
   * absolute result only if it stays inside `base`; otherwise returns `null`.
   *
   * Normalization and containment are fused so there is no separate check to
   * forget: a `..`-laden or absolute `path` that would climb above `base` is
   * rejected (returns `null`) rather than silently escaping. Pass the root as
   * `base` for a plain trusted join.
   *
   * Purely lexical - it does not resolve symlinks, so a symlink inside `base`
   * pointing outside it is not caught.
   *
