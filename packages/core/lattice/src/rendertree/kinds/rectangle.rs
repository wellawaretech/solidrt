...
impl Hittable for Rectangle {
    fn is_in_bounds(&self, point: XY, ctx: &HitContext) -> bool {
        ...
        match self.paint.draw_style {
            DrawStyle::Fill => {
                in_rounded_rect(point, rx, ry, rw, rh, r)
            }
            DrawStyle::Stroke => {
                let in_outer = in_rounded_rect(point, rx - half_sw, ry - half_sw,
                    rw + half_sw * 2.0, rh + half_sw * 2.0, r + half_sw);
                let in_inner = in_rounded_rect(point, rx + half_sw, ry + half_sw,
                    rw - half_sw * 2.0, rh - half_sw * 2.0, (r - half_sw).max(0.0));
                in_outer && !in_inner
            }
            DrawStyle::StrokeAndFill => {
                in_rounded_rect(point, rx - half_sw, ry - half_sw,
                    rw + half_sw * 2.0, rh + half_sw * 2.0, r + half_sw)
            }
        }
    }
