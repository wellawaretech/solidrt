...
- `layout-slide.tsx` - the `layout` transition, the companion to exit: rows declaring it slide from the box they had to the box a layout gives them, position and size, instead of jumping - tap a row to expand it (it grows to fit its detail text while the rows below slide down), its "remove" mark to remove it (its exit plays where it was while the rows below slide up), the header to shuffle the order (a `<For>` reorder), the footer to add one back. No per-frame JS, no hand-animated `y` or `height`.

## Pointer input
- `gestures.tsx` - the discrete recognizers test bed: a swipe card (`createSwipe`, follows the finger, reports the direction and speed, snaps back when too slow), a long-press tile (`createLongPress`, arms on a hold, then drags: the reorder idiom), a double-tap target beside a plain tap (the single tap fires late only where a double-tap is registered: the arena's pend/defer relation), a pan target printing the release velocity, and a pointer feed binding `swipe("Left")`, `longPress` and `doubleTap` on an input map. Every state is on screen and in the log, so `/input` sequences verify it from the tree.
...
## Gamepads
- `gamepad.tsx` - the pad test bed: every connected pad's raw `gamepads()` snapshot (slots, buttons, axes) and two players seated by `gamepad.next()` - press any button on a pad to join player 1, another pad joins player 2 - each an input map (`move` on the left stick and dpad, `zoom` on the triggers, `jump` on south) printing its values live and walking a marker with `move`. Nothing moves until a pad does.
