...
// camera, plus two `<View3d>`s - a top-down ORTHOGRAPHIC map (`ortho` in
// the view's `camera`) and a side silhouette drawn with an
...
// A view is a Scene child: it renders its own `<texture>` leaf where it
// sits, so the two stack in the column beside the scene's leaf, and it
// dies with the scene.
//
...
function App() {
  ...
  return (
    <window>
      <view width={pct(100)} height={pct(100)} designSize={[MAIN + SIDE, MAIN]} flexDirection="row">
        <Scene
          width={MAIN}
          height={MAIN}
        ...
        >
          ...
          {/* The two views' leaves stack beside the scene's: a laid-out
              element among Scene children renders where it sits, and the
              views inside it stay Scene children through context. */}
          <view flexDirection="column" width={SIDE} height={MAIN}>
            {/* The map: straight down from y = 10, world -z toward the top
                of the leaf, seven world units across at any depth. Its mask
                also admits the marker layer, and `fog={null}` keeps it clear
                while the main view fades to the horizon. */}
            <View3d
              width={SIDE}
              height={SIDE}
            ...
            />
            {/* The silhouette: every mesh drawn flat amber, from the side.
                The floor is edge-on here, a single line at the bottom. */}
            <View3d
              width={SIDE}
              height={SIDE}
