...
- Focus navigation: `createFocusNav` moves real focus across `focusable`
  ...
  `navigate`, sequentially on `cycle` (reading order, wrapping), activating
  on `select`: three input-map actions (`uiActions`), never a device read.
  Bare, it binds the standard set (`uiBindings`: arrows, Tab, Enter, Space,
  remote center; dpad, left stick, south on every pad) on its own map,
  `nav.input`; or pass `input`. Spread `nav.handlers` on the window. A
  focused press control activates only through `select` (createPress reads
  no key) and draws Button's ring under `policy.focusRing` (true when a
