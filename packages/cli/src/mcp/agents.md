# Debugging a running app over MCP

Read this before investigating a bug in a running app, or before driving the
app over MCP to verify a change. The same control API is reachable without
MCP from a shell or a CI step: see src/server/agents.md.
