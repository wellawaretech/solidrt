...
let EVENT_ARG = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    action: z.enum(["connect", "set", "disconnect"]),
    slot: z
      .number()
      .int()
      .min(0)
      .max(15)
      .optional()
      .describe("The pad's slot: required for set and disconnect; on connect the slot to seat in (default: the lowest free one, as a physical pad takes)"),
    name: z.string().optional().describe("connect only: the pad's reported name (default 'Synthetic gamepad')"),
    buttons: z
      .array(z.string())
      .optional()
      .describe(
        "set only: the buttons held from now on (SDL positional names: south, east, west, north, back, guide, start, leftStick, rightStick, leftShoulder, rightShoulder, dpadUp, dpadDown, dpadLeft, dpadRight); omitted = none",
      ),
    axes: z
      .record(z.string(), z.number())
      .optional()
      .describe("set only: axis values held from now on (leftX, leftY, rightX, rightY in -1..1, y down; leftTrigger, rightTrigger in 0..1); omitted = all at rest"),
    holdMs: z.number().int().min(0).max(5000).optional().describe("set only: hold this state that long, then return the pad to rest in the same call"),
    delayMs: DELAY_ARG,
  }),
])
...
let TOOLS: {
  ...
  },
  {
    ...
    annotations: READ_ONLY,
    description:
      "Performance statistics from a running app client. Start with `window`: a summary of the frames that changed the picture in the last window_ms (default 5000, max 10000), or the last window_frames of them (the window to use under a frozen clock): tree rebuilds, plus GPU content changes presented without one (a layer write, a shader param, an upload: a sprite or shader app's every frame, where the critical path is the render handler alone) - frames, p50Ms/p95Ms/maxMs of the JS-thread critical path per frame (render handler + layout + postLayout + paint + hover), slowFrames (frames over the refresh period, periodMs), captureFrames (frames a get_snapshot readback stalled - tooling time, left out of the percentiles, slowFrames and worst, and marked as such in the slow-frame log line), the window's totals backdropsPrepainted (summed over its frames) and nodesPaintedMax, and `worst`, the single most expensive frame with its ageMs, phase breakdown (jsMs/layoutMs/postLayoutMs/paintMs/hoverMs) and that frame's own layout activity (paraShapes, measureCalls, dirtiedNodes, cacheGets/cacheHits, nodesPainted, backdropsPrepainted). This is where jank shows: quote the window's missedPresents (presents the display missed while a next frame was demanded - each one a refresh that repeated the old frame, the hitch the eye sees) as the primary jank figure, and the other figures as its cause (slowFrames = JS thread, gpuFrameExecMsPerFrame/fenceTimeoutsPerSec = GPU, rasterCmdMsPerSec = raster side work). It counts what everything else is blind to: a frame missing its slot on the raster thread or in GPU execution leaves the JS critical path, fps and every per-second average reading clean. Typical flow: mute_user_input, then send_input a burst (typing, a drag), then get_stats - mute first whenever you are measuring, or the human's own input is in the sample and nothing in the payload says so - `frames: 0` means nothing changed the picture in the window (idle app), which is different from all-fast. The window also carries the GPU figures when it spans 2+ frames: missedPresents (a count over the window, not a rate; idle gaps never count, only gaps while a next frame was demanded), fenceTimeoutsPerSec, gpuPassesPerFrame (per presented frame), gpuPassIssueMsPerFrame, gpuPassExecMsPerFrame, gpuFrameExecMsPerFrame, rasterCmdMsPerSec - read these instead of differencing the cumulatives yourself. timeMs (client monotonic clock) and frame (present index) stamp the payload so two samples can be differenced. Then the smoothed figures: fps, CPU%, memory, smoothed JS/layout/paint/hover frame times (ms), setProperty writes per frame, demand-gate reuse/skip counts per second, and live texture count. Layout-activity counters cover the last full rebuild, raw: nodes (live node count, mounted AND detached), mountedNodes/orphanNodes (live at query time: nodes reachable from the root vs not - orphans growing at a stable tree shape mean an unmount leak; absent when no engine is running), measureCalls (text measures; mostly cache hits, cheap), paraShapes (paragraphs actually shaped, i.e. words the shared word cache did not have; the expensive signal - high layoutMs with near-zero paraShapes means the cost is not text shaping), wordHits (words answered from the shared word cache; hits high and paraShapes near zero on a text change means only the changed words were reshaped), dirtiedNodes (layout caches cleared by property writes since the previous rebuild; how much of the tree a write burst invalidated), cacheGets/cacheHits (layout-cache lookups during the rebuild; a hit on a container skips its whole subtree, so a healthy incremental rebuild shows a near-100% hit rate - a low rate at scale means the layout cache is being defeated), nodesPainted (nodes the latest frame's paint walk entered, 0 when that frame reused the display list - the last rebuild's count is in `window.worst`; mountedNodes minus this is what viewport culling skipped - a long scroller should paint a near-constant number of nodes however long its content), backdropsPrepainted (backdropFilter panels the latest walk re-filtered ahead of a fading ancestor's opacity group so the glass keeps its blur through the fade; nonzero only while such a fade runs, one extra backdrop capture per panel per frame; 0 on a reuse frame like nodesPainted, so read the window's total, `window.backdropsPrepainted`), damagePx (the latest rebuild's resolved screen-damage area in logical px^2, the union of every changed node's old and new window extents: window area on a fully damaged frame - resize, first paint, or a change without a trustworthy extent - and a small number when only a small region changed; a small animation reporting window-sized damagePx means its damage is not being contained), paintOps (the display-list ops the latest rebuild's paint walk recorded, also in window.worst for the worst frame: draws, of which paragraphs are text's paragraph draws (one per line run of same-styled words; a justified line or a style change costs one per word); clips, of which roundedClips; saveLayers; and blends and gradients, the paints that leave a tiled GPU's cheap path - the counts that name a paint cost on Android without a reload per hypothesis; ops inside a reused repaint-boundary recording are not counted). The window's `targets` list splits the pass figures per draw target that rendered in it (label as created, id 0 = node shaders): passesPerFrame, gpuPassIssueMsPerFrame, gpuPassExecMsPerFrame (absent without timer queries) and verticesPerFrame (vertices, or indices on an indexed draw, submitted per presented frame: a vertex-bound pass shows here before any timer does; fill still needs subtraction). GPU-side health, read live at query time (absent when no engine is running): rasterQueue (raster commands sent but not yet executed at the instant of the query, including the one executing; the frame command blocks on vsync in it, so 1 while frames flow is normal - it is a backlog signal only when it climbs across queries while fps drops; a persistently high idle reading has been seen once on a Windows client and is unexplained, so do not conclude from this field alone), partialPresents (cumulative presents that redrew only a damage patch over a preserved back buffer - partial repaint; 0 where EGL buffer age is unavailable, and climbing in step with presented frames when a small animation is being contained), idleTicks (cumulative idle frame signals emitted while the GPU had nothing queued; idleTicks racing while rasterQueue sits nonzero would mean the idle-tick gate is broken), fenceTimeouts (cumulative present-fence waits that expired instead of signaling - each one is a frame where the GPU was over budget for 100ms+ and one-frame-in-flight pacing was lost; zero on a healthy machine, climbing means the GPU is the bottleneck right now), missedPresents (cumulative missed presents, the window figure's raw counter - diff your own samples with it; the window's count is the one to quote), gpuPasses/gpuPassIssueMs/gpuPassExecMs (cumulative shader/pipeline target renders on the raster thread, the wall time the raster thread spent issuing them, and the GPU-side time executing them, all in whole ms - diff two queries to get a rate; passes racing far ahead of frames means redundant target re-renders, the failure mode where fps and frameMs look healthy while the raster thread drowns; issue and exec are different clocks: a pass with a heavy fragment shader is cheap to issue and expensive to execute, so a busy GPU with a small issue figure is normal, and gpuPassExecMs is the number to compare against the refresh period. gpuPassExecMs comes from GL timer queries and lags the pass by a frame or two; it is absent, not 0, when the client's context has none or its driver failed the startup attribution self-test - some tiled GPUs book deferred pass execution to whichever query is open, so their split would mislead; measure by subtraction there, differencing frame counts against timeMs), gpuFrameExecMs (cumulative GPU-side time executing the window draw of each presented frame - the display list plus any window shader, excluding the pass flush and the present - from the same timer queries, same absence rule; gpuFrameExecMsPerFrame in the window is the number to hold against periodMs: near or above it, the GPU is the bottleneck and fenceTimeouts follow, while a healthy jsMs says nothing about it), rasterCmdMs (cumulative wall time in whole ms the raster thread spent executing non-frame commands - texture uploads, readbacks, offscreen rasterizations, shader compiles, param writes and the target re-renders they trigger; the work frameMs never sees, so rasterCmdMs growing much faster than frames are presented means the raster thread is drowning in side work even if every counter above looks calm).",
    inputSchema: {
      window_ms: z
        .number()
        .int()
      ...
      window_frames: z
        ...
        .max(600)
        .describe(
          "The last n frames that changed the picture, in place of a time window: the window for a frozen clock (set_time_scale 0), where stepped frames fall out of a time window while you read it (max 600)",
        )
        .optional(),
      client: CLIENT_ARG,
    },
  ...
  },
  {
    ...
    annotations: READ_ONLY,
    description:
      "Snapshot of a running app client\'s render tree, the /tree control endpoint verbatim: node id, kind, window-relative box (x, y, width, height), text content, and children. Use it to verify what the app actually rendered and where. The per-node record - props (JSX names, off-default values only), quad under a transform, exiting/exit on a removal cascade, slide mid-slide, detached, childCount - is documented once, in agents/debugging.md under \'The control API without MCP\'; read the /tree entry there for what each field means and how to read animations off it with a frozen clock. Whole trees get large: prefer `query` to find nodes by kind or text first, then `root` + `depth` (+ props) to inspect the region around a match; a node whose children were cut off by `depth` carries `childCount`, descend into it with root=<its id>.",
    inputSchema: {
      root: z
        .number()
        .int()
  ...
  },
  {
    ...
    annotations: SETS_STATE,
    description:
      "Pause the dev server's reload-on-save until resume_watch, so your half-finished saves are not pushed to the user's screens while you edit, or while you GENERATE files the app reads - the assets/ tree is watched too, so a data-prep script writing into it rebuilds and reloads the app on every write. Call it before an edit burst or a generating script; when the edits are done, reload, then resume_watch. Changes saved while paused are not replayed on resume: reload is what pushes them. The pause lifts on resume_watch, when the dev server goes away, or when this bridge exits. ALWAYS resume when you are done: while paused, the human's own saves reach nothing. Returns `{ ok, watchPaused }`.",
    inputSchema: {},
  },
  {
    name: "resume_watch",
...
async function callTool(name: string, args: any): Promise<ControlResult> {
  switch (name) {
    ...
    case "get_stats": {
      ...
      if (typeof args?.window_frames === "number") params.set("frames", String(args.window_frames))
      if (typeof args?.client === "number") params.set("client", String(args.client))
      let qs = params.toString()
