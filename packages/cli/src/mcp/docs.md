...
`set_time_scale`), reload and load, a mute on the user's own input while the agent
measures or tests (`mute_user_input`/`unmute_user_input`), a pause on
reload-on-save while it edits (`pause_watch`/`resume_watch`), and any debug
commands the app itself registers.
...
project root (src/mcp/agents.md in this package lists the file per client).
...
without MCP (src/server/agents.md).
