...
- **Idle spin** (fixed): the engine loop slept 1 ms and re-selected whenever `PendingOps` was held while the rquickjs spawner was empty. Servers/sockets/timers did NOT pay it (their holds pair with a `ctx.spawn` future, so `runtime.idle()` parks; measured 0 wakeups/s headless) - the real victim was every idle GUI client, whose `srt:events` listeners hold with no spawner future: a permanent 1 kHz wakeup on the JS thread. The loop now parks on the exec channel and the release notification once the runtime has drained.
