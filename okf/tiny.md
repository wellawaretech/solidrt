...
- renderer.ts `insertNode`: a same-parent move (a `<For>` reorder is an insertBefore of an attached node) never removes the node's previous entry from the JS mirror `parent.children`, while the native tree does (`retain`), so `getNextSibling` hands Solid stale anchors and later inserts land in the wrong place (a row appended after a rotate landed first, the footer drifted into the list in probes/layout-slide-probe.tsx). Splice the existing entry out before inserting.
