...
Both sections were cut out when okf was restructured, so this record carries no
loose knowledge and no open work:

- the coalescing rules (deltas sum, no reordering around down/up, one hit test
  per pointer per frame, no resampled deltas) ->
  [what must not be collapsed](../notes/pointer-coalescing-traps.md)
- the unsettled EMA / filtering-placement questions ->
