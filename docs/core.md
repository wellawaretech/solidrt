...
Renders text. Children are the text content: strings, and `<span>` runs that override the paragraph's style for part of it. Paragraph-level props: `textAlign`, `maxLines` with `textOverflow` (`"clip"`, `"ellipsis"`, or any string to draw as the ellipsis), and `overflowWrap` (`"anywhere"`, the default, splits a word wider than the line at grapheme boundaries; `"normal"` keeps it whole and lets it overflow), `textIndent` (first-line indent in pixels; negative hangs: the first line starts at 0 and the lines after it are indented by the magnitude), and `textWrap` (`"wrap"`, the greedy default; `"balance"` evens the line lengths at the same line count; `"pretty"` keeps a lone last word from standing alone).

`textDecoration="underline"` underlines the text in its own color, on `<text>` or on a `<span>` (a link). The line runs through descenders (no skip-ink) and covers the spaces between words; position and thickness are the font's own, overridable with `textUnderlineOffset` (pixels from the baseline to the top of the line) and `textDecorationThickness` (pixels). A font Impeller resolves through the system fallback has no metrics we can read and gets the shipped Noto values.

Any other element child (`<view>`, `<texture>`, `<rect>`, `<path>`, ...) is an inline atom: it flows with the words as one unbreakable unit, sized shrink-to-fit by its own layout (like an inline block), bottom on the line's baseline, and painted and hit-tested where it lands. It is a box, not a run: the paragraph's font props do not cascade into it. With `float="left" | "right"` an atom leaves the flow: it sits against that side at the top of the line where it occurs and the lines it overlaps wrap around its margin box (image runaround, drop caps); the text's height includes it. `clear="left" | "right" | "both"` on an atom starts a new line below the earlier floats on that side.
...
`blendMode` (the full Skia set: `"plus"`, `"screen"`, `"multiply"`, ...) is how several GPU passes composite in the tree. Stack absolutely-positioned `<texture>` elements - a base pass, then an additive `blendMode="plus"` pass over it - instead of writing a shader that samples both targets. Texture alpha is premultiplied, so additive modes need no manual premultiplication. Within one pipeline's own draw, `createPipelineTexture`'s `blend: "add"` option makes overlapping geometry accumulate additively and `blend: "multiply"` makes it scale what is already in the target (all four channels: alpha 1 with rgb below 1 darkens, a projected shadow; a uniform factor fades the existing premultiplied pixels - so a strength-weighted shadow is `vec4(mix(vec3(1.0), shadowColor, strength), 1.0)`, not alpha = strength, which fades instead). Both are order-independent, so no sorting; a depth-tested pass pairs either with `depthWrite: false`, stated explicitly - neither option implies the other. `blend: "alpha"` composites over (`ONE, ONE_MINUS_SRC_ALPHA`, premultiplied output: `vec4(color * a, a)`), classic translucency; it is the one order-dependent mode - the result follows draw-list order, so translucent geometry lands back-to-front by `before`/`setDrawOrder` or a sorting layer above, normally after the opaques with `depthWrite: false`; nothing sorts for you. Without `blend` a target's draw runs with GL blending disabled and overwrites.
...
- **Values are non-linear RGBA8, with no color-space concept.** Every texture and target holds 8-bit RGBA UNORM exactly as the shader wrote it; nothing converts to or from linear light. So `filter: "linear"` averages and the `blend` modes accumulate non-linear values - the usual approximation, stated here so shaders written today stay correct if a format vocabulary ever arrives.
