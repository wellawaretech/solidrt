...
```js
let screen = createShader(src, 320, 200, { iTime: 0 }, { filter: "nearest" })
...
In every target create (`createShader`, `createPipeline`, `createShaderTarget`), `params` is its own argument ahead of the options - pass `null` when there are none. It seeds the live uniform channel the `params` prop and `setShaderParams` drive afterwards; the options bag holds only fixed creation-time configuration (`textures`, sampling, `label`, draw state where it applies).
