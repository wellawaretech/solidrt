...
pub(crate) fn init_write<'js>(ctx: &Ctx<'js>, flux: &Object<'js>) {
  let write_fn = Function::new(
    ctx.clone(),
    MutFn::from(|ctx: Ctx<'_>, path: String, data: Value<'_>| -> rquickjs::Result<Promised<_>> {
      let bytes = if let Some(s) = data.as_string() {
        ...
        return Err(
          ctx.throw(
            rquickjs::String::from_str(ctx.clone(), "Flux.write: data must be string or Uint8Array")
