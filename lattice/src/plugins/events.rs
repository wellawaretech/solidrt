...
use flux::rquickjs::{function::MutFn, Ctx, Function, IntoJs, JsLifetime, Persistent, Value};

// The UI event surface: the `on` / `once` exports of the `srt:events` module
// plus sticky-event policy. This sits on top of flux's neutral event bus
// (register_listener + emit_event); flux itself has no notion of which events
// exist or of stickiness. A future flux:process would expose its own surface
// the same way.
...
// Stores the sticky-cache userdata. Runs as an engine plugin so the cache is
// present before any event is emitted, independent of whether JS has imported
// `srt:events` yet (native code emits regardless).
pub fn init(ctx: &Ctx<'_>) {
  ctx.store_userdata(StickyCache::default()).expect("store sticky cache");
}

// The `srt:events` module: the `on` / `once` subscription surface. Reads the
// sticky cache and flux's listener registry from userdata, both installed
// eagerly by `init`, so the module body carries no state of its own.
