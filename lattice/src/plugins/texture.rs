...
fn throw_str(ctx: &Ctx<'_>, msg: &str) -> flux::rquickjs::Error {
  ctx.throw(
    flux::rquickjs::String::from_str(ctx.clone(), msg)
      .expect("create error string")
      .into(),
  )
}

fn decode_image_impl<'js>(
  ctx: Ctx<'js>,
  data: TypedArray<'js, u8>,
) -> flux::rquickjs::Result<Object<'js>> {
  let raw = data
    .as_raw()
    .ok_or_else(|| throw_str(&ctx, "decodeImage: detached buffer"))?;
  let bytes = unsafe { std::slice::from_raw_parts(raw.ptr.as_ptr(), raw.len) };
  let img = image::load_from_memory(bytes)
    .map_err(|e| throw_str(&ctx, &format!("decodeImage: {e}")))?;
...
pub fn init(ctx: Ctx<'_>, atx: AlloyContext) {
  ...
  let create_texture = Function::new(
    ...
    move |ctx: Ctx<'_>, data: TypedArray<'_, u8>, width: u32, height: u32| -> flux::rquickjs::Result<u64> {
      let raw = data
        .as_raw()
        .ok_or_else(|| throw_str(&ctx, "createTexture: detached buffer"))?;
      let expected = (width as usize) * (height as usize) * 4;
      if raw.len != expected {
        return Err(throw_str(
          &ctx,
          &format!("createTexture: expected {expected} RGBA8 bytes, got {}", raw.len),
        ));
      }
